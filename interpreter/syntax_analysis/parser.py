...
class Parser(object):
    ...
    def declarations(self):
        ...
        while self.current_token.type in [CHAR, FLOAT, DOUBLE, SHORT, INT, LONG, HASH]:
    ...
    def function_body(self):
        ...
        while self.current_token.type != RBRACKET:
            if self.current_token.type in (CHAR, SHORT, INT, LONG, FLOAT, DOUBLE, UNSIGNED):
    ...
    def declaration_list(self):
        ...
        while self.current_token.type == (CHAR, SHORT, INT, LONG, FLOAT, DOUBLE, UNSIGNED):
    ...
    def check_selection_statement(self):
        return self.current_token.type in (IF,SWITCH,CASE)
    ...
    def selection_statement(self):
        ...
        if self.current_token.type == IF:
            ...
            self.eat(SWITCH)
            self.eat(LPAREN)
            variable = self.primary_expression()
            self.eat(RPAREN)
            self.eat(LBRACKET)
            body = []
            while self.current_token.type != RBRACKET:
    ...
    def check_assignment_expression(self):
        while self.current_token.type == MUL_OP:
            self.eat(self.current_token.type)
    ...
    def unary_expression(self):
        ...
        elif self.current_token.type == SIZE_OF:
            return self.builtin_fucntions()
        else:
            return self.postfix_expression()

    def builtin_fucntions(self):
        token = self.current_token
        self.eat(SIZE_OF)
        self.eat(LPAREN)
        node = self.expression()
        self.eat(RPAREN)
        return BuiltinFunctionStmt(op=token, expression=node, line=self.lexer.line)
    ...
    def type_spec(self):
        ...
        if token.type in (CHAR, SHORT, INT, LONG, FLOAT, DOUBLE, UNSIGNED):
            array_of_types = []
            while self.current_token.type in (CHAR, SHORT, INT, LONG, FLOAT, DOUBLE, UNSIGNED):
                array_of_types.append(self.current_token.type)
                self.eat(self.current_token.type)

            if UNSIGNED in array_of_types and SHORT in array_of_types:
                token.type = UNSIGNED_SHORT
                token.value = 'unsigned_short'
            elif UNSIGNED in array_of_types and INT in array_of_types:
                token.type = UNSIGNED_LONG
                token.value = 'unsigned_int'
            elif UNSIGNED in array_of_types and LONG in array_of_types:
                token.type = UNSIGNED_LONG
                token.value = 'unsigned_long'
            elif UNSIGNED in array_of_types and CHAR in array_of_types:
                token.type = UNSIGNED_LONG
